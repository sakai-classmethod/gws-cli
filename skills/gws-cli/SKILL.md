...
```bash
uv run gws-cli calendar attachments <event-id> [--calendar-id <calendar-id>]
```
...
```bash
uv run gws-cli docs get <doc-id> [--format plain|md] [--section transcript|notes]
```
...
```bash
uv run gws-cli calendar attachments <event-id>
```
...
```bash
# 文字起こしのみ抽出（トークン節約）
uv run gws-cli docs get <fileId> --section transcript

# メモ（Gemini 要約）のみ
uv run gws-cli docs get <fileId> --section notes

# 全文
uv run gws-cli docs get <fileId>

# Markdown 形式で取得
uv run gws-cli docs get <fileId> --format md --section transcript
```
...
- 作業ディレクトリを gws-cli リポジトリに変更してから `uv run` を実行すること
